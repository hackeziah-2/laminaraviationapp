...
const APPROVAL_TYPES = ['ATOC', 'AMOC', 'ATOC VALIDITY NEPAL', 'FSTD', 'AMDC'] as const;
...
export function OrganizationalApprovals() {
  ...
  const [approvals, setApprovals] = useState<Approval[]>([
    { id: 1, certificate: 'ATOC', number: '2020-01', expiry: '16 Aug 30', fileLink: '#' },
    { id: 2, certificate: 'AMOC', number: '184-20', expiry: '31 Oct 27', fileLink: '#' },
    { id: 3, certificate: 'ATOC VALIDITY NEPAL', number: '', expiry: '29 Oct 27', fileLink: '#' },
    { id: 4, certificate: 'FSTD', number: 'FSTD-2020-01', expiry: '16 Aug 30', fileLink: '#' },
    { id: 5, certificate: 'AMDC', number: '07-2024', expiry: '22 Sep 26', fileLink: '#' },
  ]);
  ...
  const typeCounts = {
    ...
    ...Object.fromEntries(certificateTypes.map(type => [type.toLowerCase(), approvals.filter(a => a.certificate === type).length]))
  };
  ...
  const handleSaveDocument = () => {
    ...
    const expiryFormatted = formData.expiryDate
      ? new Date(formData.expiryDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: '2-digit' })
      : '—';
    setApprovals((prev) => [
      ...prev,
      {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          <h1 className="text-gray-900 text-xl sm:text-2xl">Organizational Approvals</h1>
          <p className="text-gray-500 mt-1 text-sm sm:text-base">Operational certificates and organizational approvals tracking</p>
        </div>
      ...
      <div className="text-white px-4 sm:px-6 py-3 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0" style={{ backgroundColor: '#2563EB' }}>
        <span className="tracking-wide text-sm sm:text-base">ORGANIZATIONAL APPROVALS</span>
            ...
            >
              ...
              {APPROVAL_TYPES.map((t) => (
                <option key={t} value={t.toLowerCase()}>
                  {t} ({(typeCounts as Record<string, number>)[t.toLowerCase()] ?? 0})
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="text-gray-600">
        Showing {filteredApprovals.length > 0 ? startIndex + 1 : 0} to {Math.min(startIndex + itemsPerPage, filteredApprovals.length)} of {filteredApprovals.length} approvals
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-3 text-left text-xs text-gray-600 uppercase tracking-wider">CERTIFICATE</th>
                <th className="px-6 py-3 text-left text-xs text-gray-600 uppercase tracking-wider">NUMBER</th>
                ...
                <th className="px-6 py-3 text-left text-xs text-gray-600 uppercase tracking-wider">FILE</th>
              </tr>
            </thead>
              ...
              {paginatedApprovals.length > 0 ? (
                paginatedApprovals.map((approval) => (
                  <tr key={approval.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-3.5 text-gray-900 font-medium">{approval.certificate}</td>
                    <td className="px-6 py-3.5 text-gray-900">
                      {approval.number || <span className="text-gray-400">—</span>}
                    </td>
                    <td className="px-6 py-3.5 text-gray-900">{approval.expiry}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-white/15 backdrop-blur-[4px]" onClick={closeModal} />
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">Add Approval</h2>
              ...
              <div>
                ...
                {uploadFile || uploadFileName ? (
                  <div className="flex items-center gap-2 p-3 border border-gray-300 rounded-md bg-gray-50">
                    ...
                    <button
                      type="button"
                      onClick={() => { setUploadFile(null); setUploadFileName(''); }}
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
                    ...
                    />
                    <label htmlFor="org-approval-file" className="cursor-pointer flex flex-col items-center">
